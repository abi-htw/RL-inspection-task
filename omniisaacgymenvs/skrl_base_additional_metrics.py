...
class Agent:
    def __init__(self,
        ...
        self._track_accuracy = collections.deque(maxlen=100)
        self._track_stability = collections.deque(maxlen=100)
        self._track_job_successes = collections.deque(maxlen=100)
        self._track_job_rejections = collections.deque(maxlen=100)
        self._make_span_que = collections.deque(maxlen=100)
        self._cumulative_rewards = None
        self._cumulative_timesteps = None
        self._cumulative_accuracy = None
    ...
    def record_transition(self,
        ...
        if self._cumulative_rewards is None:
            ...
            self._cumulative_accuracy = torch.zeros_like(rewards, dtype=torch.float32)
            self._cumulative_stability = torch.zeros_like(rewards, dtype=torch.float32)
            self._cumulative_job_succeses = torch.zeros_like(rewards, dtype=torch.float32)
            self._cumulative_job_rejections = torch.zeros_like(rewards, dtype=torch.float32)
            self._make_span_time = torch.zeros_like(rewards, dtype=torch.float32)


        self._cumulative_rewards.add_(rewards)
        self._cumulative_timesteps.add_(1)
        self._cumulative_accuracy.add_(infos['accuracy'].unsqueeze(1))
        self._cumulative_stability.add_(infos['stability'].unsqueeze(1))
        self._cumulative_job_succeses = infos['job_successes'].unsqueeze(1)
        self._cumulative_job_rejections = infos['job_rejections'].unsqueeze(1)
        self._make_span_time = infos['waiting_time']
        self._mean_consecutive_succ = infos['consecutive_successes']
        self._successes = infos['successes']
        ...
        # print(finished_episodes[0] if finished_episodes)
        if finished_episodes.numel():

            # storage cumulative rewards and timesteps
            self._track_rewards.extend(self._cumulative_rewards[finished_episodes][:, 0].reshape(-1).tolist())
            self._track_timesteps.extend(self._cumulative_timesteps[finished_episodes][:, 0].reshape(-1).tolist())
            # print(finished_episodes[0].item())
            # Logic to check the number of successes of the finished episodes and assigning an accuracy of 0 to the envs where success are less than a specific value
            # print(self._cumulative_rewards.shape)
            # print(self._cumulative_accuracy)
            # for env, succ, in enumerate(infos["successes"]):
            #     for eps in finished_episodes[0]:
            #         if eps.item() == env and succ.item() < 3:
            #             self._cumulative_accuracy[env]= 0


            # mean_accuracy = torch.div(self._cumulative_accuracy[finished_episodes][:, 0].squeeze(1), self._cumulative_timesteps[finished_episodes][:, 0].squeeze(1))
            mean_accuracy = torch.div(self._cumulative_accuracy[finished_episodes][:, 0], self._cumulative_timesteps[finished_episodes][:, 0])
            job_successes = self._cumulative_job_succeses[finished_episodes][:, 0]
            job_rejections = self._cumulative_job_rejections[finished_episodes][:, 0] 
            make_span = self._make_span_time[finished_episodes][:, 0] 
            mean_stability = torch.div(self._cumulative_stability[finished_episodes][:, 0], self._cumulative_timesteps[finished_episodes][:, 0])




            # print(mean_accuracy)
            self._track_accuracy.extend(mean_accuracy.reshape(-1).tolist())
            self._track_job_successes.extend(job_successes.reshape(-1).tolist())
            self._track_job_rejections.extend(job_rejections.reshape(-1).tolist())
            self._make_span_que.extend(make_span.reshape(-1).tolist())
            self._track_stability.extend(mean_stability.reshape(-1).tolist())

           

            # print(type(self._cumulative_accuracy[finished_episodes][:, 0].reshape(-1).tolist()[0]))

            # reset the cumulative rewards and timesteps
            self._cumulative_rewards[finished_episodes] = 0
            self._cumulative_timesteps[finished_episodes] = 0
            self._cumulative_accuracy[finished_episodes] = 0
            self._cumulative_job_succeses[finished_episodes] = 0
            self._cumulative_job_rejections[finished_episodes] = 0
            self._make_span_time[finished_episodes] = 0
            self._cumulative_stability[finished_episodes] = 0
        ...
        if self.write_interval > 0:
            ...
            self.tracking_data["Consecutive_Succ / Consecutive_Succ (mean)"].append(self._mean_consecutive_succ.item())
            self.tracking_data["Successes / timestep"].append(torch.mean(self._successes).item())

            # self.tracking_data["Tiemstep / Job Rejections (mean)"].append(torch.mean(infos['job_rejections']).item())

            # print(torch.mean(infos['job_rejections']).item())
            # self.tracking_data["Accuracy / Timestep"].append(torch.mean(infos['accuracy']).item())


            if len(self._track_rewards):
                # print(type(self._track_accuracy), type(self._track_rewards), type(self._track_timesteps))
                track_rewards = np.array(self._track_rewards)
                track_timesteps = np.array(self._track_timesteps)
                trak_accuracy = np.array(self._track_accuracy)
                trak_job_successes = np.array(self._track_job_successes)
                trak_job_rejections = np.array(self._track_job_rejections)
                trak_make_span = np.array(self._make_span_que)
                trak_stability = np.array(self._track_stability)
                ...
                self.tracking_data["Episode / Accuracy (max)"].append(np.max(trak_accuracy))
                self.tracking_data["Episode / Accuracy (min)"].append(np.min(trak_accuracy))
                self.tracking_data["Episode / Accuracy (mean)"].append(np.mean(trak_accuracy))
                self.tracking_data["Episode / Job Successes (mean)"].append(np.mean(trak_job_successes))
                self.tracking_data["Episode / Job Rejections (mean)"].append(np.mean(trak_job_rejections))
                self.tracking_data["Episode / Makespan (mean)"].append(np.mean(trak_make_span))
                self.tracking_data["Episode / Stability (mean)"].append(np.mean(trak_stability))
                # print(np.mean(trak_job_rejections))
...
#############################with accuracy , success and rejection###################################################
